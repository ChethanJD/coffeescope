...
export function MarketCard({ quote, index }: { quote: MarketQuote; index: number }) {
  const cardRef = useRef<HTMLDivElement>(null);
  const accent = VARIETY_ACCENT[quote.variety];

  // Pointer-driven 3D tilt — subtle, spring-damped, resets on pointer leave.
  ...
  const glowBackground = useTransform(
    [glowX, glowY],
    ([gx, gy]: number[]) => `radial-gradient(320px circle at ${gx}% ${gy}%, ${accent}22, transparent 70%)`
  );
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="glass group relative overflow-hidden rounded-xl3 p-6 shadow-card sm:p-7"
      >
        {/* Pointer-follow glow */}
        <motion.div
        ...
        <div className="relative flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            >
              <Coffee className="h-5 w-5" style={{ color: accent }} />
            </span>
            <div>
              <h3 className="font-heading text-lg font-semibold text-white">
                {quote.variety}
              </h3>
              <p className="text-xs text-white/40">{quote.currency}</p>
            </div>
          </div>
        ...
        <div className="relative mt-5">
          <Sparkline data={quote.sparkline} color={accent} />
        </div>

        <div className="relative mt-5 grid grid-cols-2 gap-2">
          <ChangeBadge label="24h" value={quote.changeDaily} />
          <ChangeBadge label="7d" value={quote.changeWeekly} />
        </div>
      </motion.div>
    </motion.div>
  );
